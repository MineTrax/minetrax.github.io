...
:::info
If above `npm ci` gives you error of `ERESOVE could not resolve` then try running
...
:::tip
Pusher provide 200k messages per day in free tier, depending on your audience it might not be enough. In that case please check **[Websocket Options](./websocket-options.md)** page to learn how to setup Pusher alternatives.

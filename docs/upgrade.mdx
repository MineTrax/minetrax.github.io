...
:::caution
In some rare cases if you are getting error like `The stream or file "/var/www/minetrax/storage/logs/laravel.log" could not be opened: failed to open stream: Permission denied` then you might have to change the permission of `storage/logs` folder to `777` instead of `755`.
